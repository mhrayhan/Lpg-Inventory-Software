...
const InputField = ({ tittle, price, setQty, qty, setPrice, totalValue, name, qtyname, bgColor }) => {
  ...
  return (
    <div className='lg:w- w-full grid grid-cols-9 gap-1 '>

      <p className={`col-span-2 flex items-center ${bgColor} lg:text-xl text-sm text-white border lg:p-2 pl-1 rounded-md`}>{tittle}</p>

      <input onChange={setQty} name={qtyname} value={qty} type="number" className="input input-bordered input-md text-xl p-1 text-center col-span-2" />
